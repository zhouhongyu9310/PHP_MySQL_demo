...
$ccname = htmlspecialchars($_POST['ccname']);
$ccnumber = htmlspecialchars($_POST['ccnumber']);
$cvv = htmlspecialchars($_POST['cvv']);
$emonth = htmlspecialchars($_POST['emonth']);
$eyear = htmlspecialchars($_POST['eyear']);

$query0 = "SELECT customer_id FROM Customer INNER JOIN Reservation ON Customer.username = Reservation.customer_username WHERE Reservation.reservation_id='$reservation_id' AND Reservation.pickup_clerk IS NULL";
$st = $pdo->query($query0);
$temp = $st -> fetch();
$customer_id=$temp[0];

if ( $new == 'new'){
	$query ="UPDATE Customer SET name_on_card = '$ccname', card_number = $ccnumber, cvv = '$cvv', exp_month=$emonth, exp_year=$eyear WHERE Customer.customer_id = $customer_id";
	$st = $pdo->prepare($query);
	$st->execute();
}

$query0 = "SELECT first_name, middle_name, last_name, card_number, start_date, end_date FROM Customer INNER JOIN Reservation ON Customer.username = Reservation.customer_username WHERE Reservation.reservation_id='$reservation_id' AND Reservation.pickup_clerk IS NULL";
$st = $pdo->query($query0);
$temp = $st -> fetch();
$first_name=$temp[0];
$middle_name=$temp[1];
...
$query ="UPDATE Reservation SET pickup_clerk = '$clerk_username' WHERE reservation_id=$reservation_id";
$st = $pdo->prepare($query);
$st->execute();
...
<div class="MainMenuCus" style="padding-bottom:10px">
  ...
  <div class="line_contain" style='padding-bottom:0'><p> <b> Rental Contract </b></p> </div>
  <div class="line_contain" style='padding-bottom:0;margin-left:30px'><p> Pickup Clerk ID: <?php echo $clerk_name;?></p> </div>
...
<span class='inline_pos width_4' > <p> Pickup Clerk: <?php echo $clerk_name; ?> </p> </span>
</div>
...
<button style="margin-left:2%;height:40px;display:inline-block"> <a href='pickup1.php'> Return </a> </button>
</div>
