<?php
	...
	if ($_POST['startdate'] and $_POST['enddate']){
		if ($endtime <= $starttime){
			$error = "Reservation End Date should be late than Start Date";
			header("Location: index.php?search_tool=".$search_tool."&search_error=".$error);
			exit();	
		}

                if (($starttime+(60*60*24)) < time() ){
                        $error = "Reservation Start Date should be later than Today";
                        header("Location: index.php?reserve_tool=".$search_tool."&reserve_error=".$error);
                        exit();
                }
	}
	...
	header("Location: index.php?search_tool=".$search_tool);
	exit();
?>
