...
<div class="MainMenuCus" style="padding-bottom:10px">
    ...
    <?php
	$pdo = new PDO("mysql:dbname=Tools4Rent", "root", "root");
	$sql1 = "SELECT reservation_id, customer_username, customer_id, start_date, end_date FROM Reservation LEFT OUTER JOIN (SELECT username, customer_id FROM Customer) AS Tmptable ON Tmptable.username=Reservation.customer_username WHERE Reservation.pickup_clerk IS NOT NULL AND Reservation.dropoff_clerk IS NULL";
	$st = $pdo->query($sql1);
	$result = $st->fetchAll();
    ...
    ?>
	<form  action="dropoff2.php" method="post">
		...
		<button name="submit" type="submit" style="border-radius: 0px;border:0;margin-left:-50px;font-size:11px;height:40px;display:inline-block">Drop Off</button> 
